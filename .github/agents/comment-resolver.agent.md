...
1. **Respond with a precise plan** that maps the feedback to the appropriate layer (UI, Service, Repository, or Model) and outlines the steps to achieve a non-fragile, idiomati
      solution.
2. **Implement and verify the fix** using the project's standard workflow: gather full context, apply minimal changes, run `make check`, add/update tests, and finally remove an
      stale comment references or TODOs as per the project's cleanup rules.
